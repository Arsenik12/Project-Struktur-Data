...
obstacle_images = [stone1, stone2, stone3, stone4,
...
class SnakeGame:
    def __init__(self):
        ...
        # Inisialisasi daftar gambar sampah yang sesuai dengan posisi sampah
        self.sampah_images = []
        ...
        # Inisialisasi daftar gambar obstacle yang sesuai dengan posisi obstacle
    ...
    def reset(self):
        ...
        self.obstacle_images = []
        self.obstacle_timer = 0
        # 1,0 kanan -1,0 kiri 0,1 bawah 0,-1 atas
        self.snake_direction = (1, 0)
        self.food_position = self.getRandomFoodPos()
        # random pemilihan gambar buah
    ...
    def update(self):
        ...
        new_head_position = (
            (self.snake.head.position[0] +
             self.snake_direction[0]) % GRID_WIDTH,
            (self.snake.head.position[1] +
             self.snake_direction[1]) % GRID_HEIGHT
        )

        # kondisi jika snake nabrak ke badan sendiri atau obstacle
        if self.isEatenBySnake(new_head_position) or self.isObstacle(new_head_position):
            ...
            # addhead untuk manipulasi gerakan ular seolah-olah maju
            ...
            if new_head_position == self.food_position:  # jika snake makan makanan
                ...
                self.snake.addTrashToHistory(
                    self.sampah_images[self.sampah_positions.index(new_head_position)])
                ...
                # index dari posisi sampah yang dimakan
    ...
    def draw(self):
        ...
        while current:
            pygame.draw.rect(
                self.screen,
                (0, 255, 0),
                # menggambarkan body ular per segmen
                pygame.Rect(
                    current.position[0] * GRID_SIZE, current.position[1] * GRID_SIZE, GRID_SIZE, GRID_SIZE)
            )
            current = current.next

        # Draw food image
        self.screen.blit(
            self.food_image, (self.food_position[0] * GRID_SIZE, self.food_position[1] * GRID_SIZE))

        # Draw trash images
        # zip untuk menggabungkan 2 list
        for position, image in zip(self.sampah_positions, self.sampah_images):
            self.screen.blit(
                image, (position[0] * GRID_SIZE, position[1] * GRID_SIZE))
    ...
    def draw_start_screen(self):
        ...
        while running:
            ...
            score_text = font.render(
                f"Press Space Bar key to start", True, font_color)
        ...
        while running:
            ...
            if self.die_by_obstacle:
                description_text = font.render(
                    f"Mati tertabrak batu", True, alert_color)
            elif self.die_by_score:
                description_text = font.render(
                    f"Terlalu banyak makan sampah", True, alert_color)
            elif self.die_by_self:
                description_text = font.render(
                    f"Makan diri sendiri", True, alert_color)
            score_text = font.render(f"Score: {self.score}", True, font_color)
            restart_text = font.render(
                f"Press Space Bar key to restart", True, font_color)
            ...
            # Atur posisi awal untuk sejarah makanan dalam satu baris
            # Mengatur posisi agar gambar dimulai dari tengah
            x_offset = SCREEN_WIDTH / 2 - (GRID_SIZE * 3)
            ...
            while current:
                ...
                if food_image:
                    self.screen.blit(food_image, (x_offset, y_position))
                    x_offset += GRID_SIZE + 10  # Geser ke kanan untuk tampilan horizontal
                current = current.next

            # Draw trash history
            trash_text = "Trash Eaten History:"
            text = font.render(trash_text, True, (255, 255, 255))
            text_rect = text.get_rect(center=(SCREEN_WIDTH / 2, y_position + 50))
            self.screen.blit(text, text_rect)

            # Atur posisi awal untuk sejarah sampah dalam satu baris
            # Mengatur posisi agar gambar dimulai dari tengah
            x_offset = SCREEN_WIDTH / 2 - (GRID_SIZE * 3)
            y_position += 80
            ...
            while current:
                ...
                if trash_image:
                    self.screen.blit(trash_image, (x_offset, y_position))
                    x_offset += GRID_SIZE + 10  # Geser ke kanan untuk tampilan horizontal
                current = current.next
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_SPACE:
                        running = False  # Exit game over screen untuk restart game
